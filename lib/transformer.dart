...
class StaticMapperGenerator extends Transformer with ResolverTransformer {
    ...
    for (var directive in unit.directives) {
      if (directive is ImportDirective &&
          directive.uri.stringValue ==
              'package:redstone_mapper/mapper_factory.dart') {
  ...
  dynamic _scannClass(ClassElement clazz,
    ...
    if (clazz.supertype != null &&
      ...
      _scannClass(
          clazz.supertype.element, fields, cache, fieldIdxs, accessorIdxs);
    }
  ...
  void _scannAccessor(List<_FieldInfo> fields, PropertyAccessorElement element,
      Map<String, int> accessorIdxs) {
    var field = element.metadata
        .firstWhere((f) => _isFieldConstructor(f), orElse: () => null);
    if (field != null) {
...
class _TypeCodecGenerator {
  ...
  _TypeCodecGenerator(
      this.collectionType, this.usedLibs, this.className, this.fields);
